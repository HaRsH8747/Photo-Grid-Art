...
public class Shape {
    ...
    public void setMaxMinScales(float scaleBitmap) {
        if (this.isScrapBook) {
            ...
            this.minScale = this.scaleFactor * scaleBitmap;
        }
        if (this.isScrapBook) {
    ...
    public void bitmapMatrixTranslate(float dx2, float dy2) {
        this.bitmapMatrix.postTranslate(dx2, dy2);
        if (!this.isScrapBook) {
            checkBoundries();
        }
    }
    ...
    public void checkBoundries() {
        ...
        if (this.bitmapRect.left > this.bounds.left) {
            dx2 = this.bounds.left - this.bitmapRect.left;
        }
        if (this.bitmapRect.top > this.bounds.top) {
            dy2 = this.bounds.top - this.bitmapRect.top;
        }
        if (this.bitmapRect.right < this.bounds.right) {
            dx2 = this.bounds.right - this.bitmapRect.right;
        }
        if (this.bitmapRect.bottom < this.bounds.bottom) {
            dy2 = this.bounds.bottom - this.bitmapRect.bottom;
        }
        this.bitmapMatrix.postTranslate(dx2, dy2);
    }
                ...
                if (!drawArea.contains(points2[2], points2[3])) {
                    if (!drawArea.contains(points2[4], points2[5])) {
                        if (!drawArea.contains(points2[6], points2[7])) {
                            ...
                            P.set(points2[0], points2[1]);
                            float f2 = A2.x;
                            Log.e(TAG, "A  x " + this.r0 + " y " + A2.y);
                            float f22 = B2.x;
                            Log.e(TAG, "B  x " + this.r0 + " y " + B2.y);
